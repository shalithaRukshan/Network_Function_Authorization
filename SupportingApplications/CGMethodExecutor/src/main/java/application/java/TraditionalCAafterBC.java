...
public class TraditionalCAafterBC {
    ...
    public static double BCwithCAImpl(int j) throws Exception {
        ...
        try (Gateway gateway = connect()) {
            ...
            byte[] result = contract.submitTransaction("CA", domain, req);
            logger.info("Received response from the contract: " + new String(result));
            String[] resParts = new String(result).split(("\\|\\|"));

            logger.info("Total response: " + new String(result));
...
//            String res = j + ", caafterbc," + (endtime - starttime) + "\n";
//            try {

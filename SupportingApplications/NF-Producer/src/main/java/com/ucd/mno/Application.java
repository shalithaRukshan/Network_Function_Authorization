        ...
        public void run() {
            try {
                ...
                if (isBC) {
                    ...
                    logger.info("Response for cert validation: " + new String(result));
                    out.println("ProdCert");

                    String request = in.readLine();
                    logger.info("Total request: " + request);
                    ...
                    nrfWrite.println("ProdCert");
                    String nrfCert = nrfRead.readLine();
                    result = contract.evaluateTransaction("ValidateCert", nrfCert);
                    logger.info("Response for NRF cert validation: " + new String(result));

                    nrfWrite.println("AuthValidate||token");
                    logger.info("Request sent to NRF");
                    String authResponse = nrfRead.readLine();
                    logger.info("Authresponse from NRF: " + authResponse);
